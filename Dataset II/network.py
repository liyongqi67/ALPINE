    ...
    def __init__(self,W_embedding,settings):
        ...
        with tf.name_scope('pos_loss'):
            ...
            self.pos_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self.pos_output, labels=self.y_inputs))+l2_norm * 0.00005
        with tf.name_scope('neg_loss'):
            ...
            self.neg_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self.neg_output, labels=self.y_inputs))+l2_norm * 0.00005
